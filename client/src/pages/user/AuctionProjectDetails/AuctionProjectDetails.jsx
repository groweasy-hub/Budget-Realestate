...
import { getAuctionProjectDetails } from "../../../data/user/AuctionProjectDetailsData";
...
function AuctionProjectDetails() {
  ...
  const thumbnailsToShow = project.gallery.slice(1, 4);
  const extraCount = project.gallery.length - 4;
  ...
  return (
    <PageShell>
      ...
      {/* Mobile Hero with swipe */}
      <MobileHeroWrap>
        {/* Top bar with back + save/share */}
        <MobileTopBar>
          <MobileTopBarBtn type="button" onClick={() => navigate(-1)}>
            <BackArrowIcon />
          </MobileTopBarBtn>
          <MobileTopBarRight>
            <MobileTopBarBtn type="button" onClick={() => setSaved((v) => !v)}>
              <HeartIcon $filled={saved} />
            </MobileTopBarBtn>
            <MobileTopBarBtn type="button" onClick={handleShare}>
              <ShareIcon />
            </MobileTopBarBtn>
          </MobileTopBarRight>
        </MobileTopBar>
        ...
        <MobileViewPhotos type="button" onClick={() => setShowGallery(true)}>
          <GalleryIcon />
          <span>View All Photos ({project.gallery.length * 6})</span>
        </MobileViewPhotos>

        {/* Dot indicators */}
        <MobileDotRow>
          {project.gallery.map((_, i) => (
            <MobileDot key={i} $active={i === activeImageIndex} />
          ))}
        </MobileDotRow>
      </MobileHeroWrap>
      ...
      <MobileThumbnailRow>
        {thumbnailsToShow.map((item, index) => (
          ...
          >
            <MobileThumbImg src={item.image} alt={item.imageAlt} />
            <MobileThumbLabel>{item.label}</MobileThumbLabel>
          </MobileThumbItem>
        ))}
        {/* +more tile */}
        <MobileThumbItem type="button" onClick={() => setShowGallery(true)}>
          {project.gallery[4] && (
            <MobileThumbImg
              src={project.gallery[4].image}
              alt={project.gallery[4].imageAlt}
            />
          )}
          <MobileMoreOverlay>
            <MobileMoreCount>
              +{extraCount > 0 ? extraCount : 15}
            </MobileMoreCount>
            <MobileMoreText>More</MobileMoreText>
          </MobileMoreOverlay>
        </MobileThumbItem>
      </MobileThumbnailRow>
      ...
      <MobileContent>
        {/* Breadcrumb */}
        <MobileBreadcrumb>
          <MobileCrumbLink to="/auctions">Auction</MobileCrumbLink>
          <MobileCrumbSep>›</MobileCrumbSep>
          <MobileCrumbLink to="/auctions">{project.category}</MobileCrumbLink>
          <MobileCrumbSep>›</MobileCrumbSep>
        ...
        {/* Save / Compare / Share / Brochure */}
        <MobileActionRow>
          <MobileActionBtn type="button" onClick={() => setSaved((v) => !v)}>
            <HeartIcon $filled={saved} />
            <span>{saved ? "Saved" : "Save"}</span>
          </MobileActionBtn>
          <MobileActionBtn type="button" onClick={() => setCompared((v) => !v)}>
            <CompareIcon />
            <span>{compared ? "Compared" : "Compare"}</span>
          </MobileActionBtn>
          <MobileActionBtn type="button" onClick={handleShare}>
            <ShareIcon />
            <span>{shareFeedback}</span>
          </MobileActionBtn>
          <MobileActionBtn type="button">
            <BrochureIcon />
            <span>Brochure</span>
          </MobileActionBtn>
        </MobileActionRow>
        ...
        <MobileAdvisorCard>
          <MobileAdvisorHeader>
            ...
            <MobileAdvisorMeta>
              <MobileAdvisorTag>Auction Advisor</MobileAdvisorTag>
              <MobileAdvisorName>{project.advisor.name}</MobileAdvisorName>
              <MobileAdvisorRole>{project.advisor.role}</MobileAdvisorRole>
              <MobileAdvisorExp>{project.advisor.experience}</MobileAdvisorExp>
              <MobileAdvisorRating>
                <StarIcon />
                <span>
          ...
          <MobileAdvisorButtons>
            <MobileAdvisorBtn type="button" $variant="filled">
              <HeadsetIcon />
              <span>{project.actionLabels.advisor}</span>
            </MobileAdvisorBtn>
            <MobileAdvisorBtn type="button" $variant="ghost">
              <PhoneIcon />
              <span>{project.actionLabels.callback}</span>
            </MobileAdvisorBtn>
            <MobileAdvisorBtn type="button" $variant="whatsapp">
              <WhatsAppIcon />
        ...
        <SecureAuctionCard>
          <SecureAuctionGrid>
            <VerifiedDot style={{ marginBottom: 0, width: 40, height: 40 }}>
              <ShieldCheckIcon />
            </VerifiedDot>
            <div>
        ...
        {/* Price & Auction Details */}
        <MobilePriceCard>
          <MobilePriceTitle>
            <BankBuildingIcon />
            {project.priceDetailsTitle}
          </MobilePriceTitle>
          <MobilePriceGrid>
            {project.priceAuctionDetails.map((item) => (
              <MobilePriceRow key={item.label}>
                <MobilePriceLabel>{item.label}</MobilePriceLabel>
                <MobilePriceValue>{item.value}</MobilePriceValue>
              </MobilePriceRow>
            ))}
          </MobilePriceGrid>
        </MobilePriceCard>
        ...
        {/* Overview */}
        <OverviewCard style={{ marginTop: 12 }}>
          <CardHeading>
            <HomeOutlineIcon />
        ...
        {/* Amenities */}
        <AmenitiesCard style={{ marginTop: 12 }}>
          <CardHeading>
            <AmenityHubIcon />
        ...
        {/* Location advantages */}
        <LocationCard style={{ marginTop: 12 }}>
          <CardHeading>
            <LocationListIcon />
        ...
        {/* Bank & Legal Info */}
        <BankLegalCard style={{ marginTop: 12 }}>
          <CardHeading>
            <BankBuildingIcon />
        ...
        {/* Documents */}
        <DocumentCard style={{ marginTop: 12 }}>
          <CardHeading>
            <DocumentIcon />
        ...
        {/* Similar Auctions */}
        <SimilarAuctionCard style={{ marginTop: 12 }}>
          <SimilarHeader>
            <CardHeading>
              <BuildingIcon />
        ...
        {/* Confidence card */}
        <ConfidenceCard style={{ marginTop: 12 }}>
          <ConfidenceGrid>
            <div>
      ...
      <MobileBottomBar>
        <MobileBottomBarBtn type="button">
          <GavelIcon />
          <span>{project.actionLabels.participate}</span>
        </MobileBottomBarBtn>
        <MobileBottomBarBtn type="button" $variant="ghost">
          <span>{project.actionLabels.login}</span>
        </MobileBottomBarBtn>
      </MobileBottomBar>
